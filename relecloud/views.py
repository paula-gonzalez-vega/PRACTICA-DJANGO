...
class InfoRequestCreate(SuccessMessageMixin, generic.CreateView):
    ...
    def form_valid(self, form):
        if self.request.method == 'POST':
            subject = "Información sobre " + self.request.POST.get('cruise')
            message = "Dear " + self.request.POST.get('name') + ",\n\nGracias por tu interés en " + self.request.POST.get('cruise') + ". Te escribiremos pronto con más información. Tendremos en cuenta tus notas: " + self.request.POST.get('notes') + "\n\nUn saludo,\nReleCloud Team"
            email_to = self.request.POST.get('email')
